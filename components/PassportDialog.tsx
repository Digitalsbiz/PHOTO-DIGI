...
import { IdCardIcon, XIcon, CheckIcon, PlusIcon, LoadingSpinner, SparklesIcon, ImageIcon, ChevronDownIcon } from './Icons';
...
const PassportDialog: React.FC<PassportDialogProps> = ({ isOpen, currentImageSrc, onClose, onConfirm }) => {
  ...
  // Generate options from 1 to 16, plus the 'max' option.
  const quantityOptions = useMemo(() => {
    const options: (number | 'max')[] = [];
    
    // Fill 1 through 16, as long as it fits the paper
    for (let i = 1; i <= 16; i++) {
        if (i <= maxCapacity) {
            options.push(i);
        }
    ...
    // Always add 'max' as a special option if it's not already in the list 
    // (though 'max' might be > 16)
    options.push('max');
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-gray-900 border border-gray-700 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200 max-h-[90vh] flex flex-col">
        <div className="p-6 space-y-6 overflow-y-auto custom-scrollbar">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-400">Layout optimization for printing</p>
            </div>
          </div>
          ...
          <div className="space-y-6">
            <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                    <button onClick={() => setIsEnhanceEnabled(false)} className={`relative p-3 rounded-xl border text-left flex flex-col gap-2 transition-all ${!isEnhanceEnabled ? 'bg-gray-800 border-indigo-500 shadow-[0_0_0_1px_rgba(99,102,241,1)]' : 'bg-gray-900/50 border-gray-700 hover:bg-gray-800 hover:border-gray-600'}`}>
                        <div className={`p-2 w-fit rounded-lg ${!isEnhanceEnabled ? 'bg-amber-500 text-white' : 'bg-gray-800 text-gray-400'}`}><ImageIcon /></div>
                        <div>
                            <span className={`block text-sm font-medium ${!isEnhanceEnabled ? 'text-white' : 'text-gray-300'}`}>Manual (No AI)</span>
                            <span className="text-[10px] text-gray-400 leading-tight block mt-1">Guarantees zero AI alteration.</span>
                        </div>
                    </button>
                    <button onClick={() => setIsEnhanceEnabled(true)} className={`relative p-3 rounded-xl border text-left flex flex-col gap-2 transition-all ${isEnhanceEnabled ? 'bg-indigo-900/20 border-indigo-500 shadow-[0_0_0_1px_rgba(99,102,241,1)]' : 'bg-gray-900/50 border-gray-700 hover:bg-gray-800 hover:border-gray-600'}`}>
                        <div className={`p-2 w-fit rounded-lg ${isEnhanceEnabled ? 'bg-indigo-500 text-white' : 'bg-gray-800 text-gray-400'}`}><SparklesIcon /></div>
                        <div>
                            <span className={`block text-sm font-medium ${isEnhanceEnabled ? 'text-white' : 'text-gray-300'}`}>AI Enhanced</span>
                            <span className="text-[10px] text-gray-400 leading-tight block mt-1">Auto bg removal & enhancement.</span>
                        </div>
                    </button>
                </div>
            </div>
            ...
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className={!isEnhanceEnabled ? 'opacity-40 grayscale pointer-events-none' : ''}>
                    <label htmlFor="format" className="block text-xs font-medium text-gray-400 uppercase tracking-wider mb-2">Background</label>
                    <div className="relative">
                      <select id="format" value={format} onChange={(e) => setFormat(e.target.value)} disabled={!isEnhanceEnabled} className="w-full bg-gray-900 border border-gray-700 text-white rounded-lg px-4 py-3 appearance-none focus:outline-none focus:ring-1 focus:ring-indigo-500 cursor-pointer">
                ...
                <div>
                   <label htmlFor="dimensions" className="block text-xs font-medium text-gray-400 uppercase tracking-wider mb-2">Photo Dimensions</label>
                    <div className="relative">
                      <select id="dimensions" value={dimensions} onChange={(e) => setDimensions(e.target.value)} className="w-full bg-gray-900 border border-gray-700 text-white rounded-lg px-4 py-3 appearance-none focus:outline-none focus:ring-1 focus:ring-indigo-500 cursor-pointer">
        ...
        <div className="bg-gray-950 px-6 py-4 flex justify-end gap-3 border-t border-gray-800 mt-auto">
          ...
          <button onClick={handleConfirm} className={`flex items-center gap-2 px-6 py-2 rounded-lg text-sm font-medium text-white shadow-lg transition-all hover:-translate-y-0.5 ${isEnhanceEnabled ? 'bg-indigo-600 hover:bg-indigo-500 shadow-indigo-500/20' : 'bg-amber-600 hover:bg-amber-500 shadow-amber-500/20'}`}>
            {isEnhanceEnabled ? <SparklesIcon /> : <ImageIcon />}
            <span>Generate Optimized Sheet</span>
          </button>
        </div>
      </div>
    </div>
